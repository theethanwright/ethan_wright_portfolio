...
imageElements.forEach((imageElem) => {
    ...
    const iBody = Bodies.rectangle(
     ...
     Events.on(engine, 'beforeUpdate', function(event) {
        ...
        if (engine.timing.timestamp - lastTime >= randomTime > (randomTime + 1000) &&
            engine.timing.timestamp - lastTime <= randomTime + 1000
        ) {
        ...
        if (engine.timing.timestamp - lastTime >= (randomTime + 1000)) {
